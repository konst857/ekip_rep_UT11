...
Процедура ЗагрузитьНаСервере(пИмяФайла, пДополнительныеПараметры, пЗагрузка_из_csv)
	...
	пДополнительныеПараметры.Вставить("Загрузка_из_csv", пЗагрузка_из_csv);
...
Процедура ОбработкаТаблицыФайла(пТзИзФайла,   пДополнительныеПараметры)
	...
	ИмяКолонкиЦена="Цена";
	 ...
	 ТзСвойств=ПолучитьТзСвойств(НаборыСвойств.ВыгрузитьЗначения());
	  
	 ИменоватьКолонкиТз(пТзИзФайла,  мфСтруктураЗагружаемыхКолонок);

	 
	 Если ПроверкаСтруктурыКолонокТЗ(пТзИзФайла,  мфСтруктураЗагружаемыхКолонок, Истина) Тогда
		 Возврат;
	 КонецЕсли; 
	 
	 //ДобавитьКолонку_в_ТзФайла(пТзИзФайла, ПолучитьТзХарактеристик(), "Наименование", "Характеристика");
	 
	 ДобавитьКолонку_в_ТзФайла(пТзИзФайла,НайтиСсылкуПоРеквизиту(пТзИзФайла, "Номенклатура", "Артикул", "Артикул", Отказ), "Артикул", "Номенклатура");
 
     ДобавитьСвойства(пТзИзФайла, ТзСвойств,пДополнительныеПараметры);
...
&НаСервере
Процедура ДобавитьСвойства(пТаблицаИзФайла, пТзСвойств, пДополнительныеПараметры)
	
	
	НаименованиеСвойстваТипШлема="Тип шлема (Шлемы)";
	НаименованиеСвойстваМодельШлема="Модель шлема (Шлемы)";
	//НаименованиеСвойстваМатериалШлема="Материал (Шлемы)";

	
	НаименованиеСвойстваМатериал="Материал (Одежда и обувь с характеристиками (индивидуальные))";
	
	НаименованиеСвойстваБренд="Бренд (Справочник ""Номенклатура"" (Общие))";

	НаименованиеСвойстваТип="Тип (Справочник ""Номенклатура"" (Общие))";
	НаименованиеСвойстваПол="Пол (Справочник ""Номенклатура"" (Общие))";
	
	НаименованиеСвойстваСезонность="Сезонность (Справочник ""Номенклатура"" (Общие))";
	НаименованиеСвойстваТаблицаРазмеров="Таблица размеров (Справочник ""Номенклатура"" (Общие))";
	НаименованиеСвойстваВес="Вес (Справочник ""Номенклатура"" (Общие))";
	НаименованиеСвойстваВозраст="Возраст (Справочник ""Номенклатура"" (Общие))";
	...
	Для Каждого СтрокаИзФайла из пТаблицаИзФайла Цикл
		...
		СтруктСвойств.Вставить(НаименованиеСвойстваБренд, СтрокаИзФайла.Бренд);
		СтруктСвойств.Вставить(НаименованиеСвойстваТипШлема, СтрокаИзФайла.ТипШлема);
		СтруктСвойств.Вставить(НаименованиеСвойстваМатериал , СтрокаИзФайла.Материал);
		СтруктСвойств.Вставить(НаименованиеСвойстваТип , СтрокаИзФайла.Тип);
		
		СтруктСвойств.Вставить(НаименованиеСвойстваСезонность, СтрокаИзФайла.Сезонность);
		СтруктСвойств.Вставить(НаименованиеСвойстваПол , СтрокаИзФайла.Пол);
		СтруктСвойств.Вставить(НаименованиеСвойстваМодельШлема , СтрокаИзФайла.МодельШлема);
		СтруктСвойств.Вставить(НаименованиеСвойстваТаблицаРазмеров, СтрокаИзФайла.ТаблицаРазмеров);
		СтруктСвойств.Вставить(НаименованиеСвойстваВес , СтрокаИзФайла.Вес);
		СтруктСвойств.Вставить(НаименованиеСвойстваВозраст , СтрокаИзФайла.Возраст);
	
		
	    ЗаписатьСвойстваОбъекта(СтрокаИзФайла.Номенклатура,  СтруктСвойств, пТзСвойств);
...
Процедура ЗаписатьСвойстваОбъекта(пВладелец,  пСтруктСвойств, ТзСвойсвтв)
	...
	Для Каждого Свойство из  пСтруктСвойств Цикл
			...
			Нз.Отбор.Объект.Установить(пВладелец);
			Нз.Отбор.Свойство.Установить(СвВо);
			
			НоваяЗапись = НЗ.Добавить(); 
			НоваяЗапись.Объект = пВладелец; 
			НоваяЗапись.Свойство = СвВо; 
			НоваяЗапись.Значение = ПолучитьЗначение(Свойство.значение, СвВо);
...
Функция ПолучитьЗначение(пЗначение, пСвойство)

	    ОписаниеТипа = пСвойство.ТипЗначения;
...
Процедура ВыборФайла(Элемент, ПроверятьСуществование=Ложь)
	...
	//ДиалогВыбора.Фильтр     = "Файл данных xlsx (*.xlsx)|*.txt|Файл данных (*.csv)|*.csv";
	ДиалогВыбора.Фильтр     = "Файл данных ods (*.ods)|*.ods|Файл данных (*.csv)|*.csv";

	ДиалогВыбора.Расширение = "ods";
...
мфСтруктураЗагружаемыхКолонок=Новый Структура("Бренд, Артикул, ТипШлема, Материал, Тип, Сезонность, Пол, МодельШлема, ТаблицаРазмеров, Вес, Возраст",
											  "IP_PROP231", "IP_PROP240", "IP_PROP270", "IP_PROP272", "IP_PROP273","IP_PROP274", "IP_PROP275"
											  ,"IP_PROP276", "IP_PROP300", "CP_WEIGHT", "IP_PROP251");